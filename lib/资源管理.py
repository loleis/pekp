# 本地资源管理.py - 完整修复版（整合原代码所有功能 + 小说支持 + 直播源彩色图标 + 普通文本阅读 + 修复JSON解析）
...
print("ℹ️ 本地资源管理加载成功 - 完整修复版 + JSON解析恢复")
...
class Spider(Spider):
    ...
    def _create_file_item(self, f):
        ...
        if self.is_audio_file(f['ext']):
            return {
                'vod_id': f['path'],
                'vod_name': f"🎵 {f['name']}",
                'vod_pic': self.file_icons['audio'],  # 恢复为蓝色图标
        ...
        if f['ext'] == 'txt':
                ...
                with open(f['path'], 'r', encoding='utf-8', errors='ignore') as ff:
                    ...
                    for line in lines:
                        ...
                        if ('http://' in line_lower or 'https://' in line_lower):
                            if any(ext in line_lower for ext in ['.m3u8', '.m3u', '.flv', '.mp4', '.ts', '.rmvb', '.avi']):
                                url_count += 1
                        
                        # 检测P2P链接
                        if 'ed2k://' in line_lower or 'magnet:?' in line_lower:
                            url_count += 1
            except Exception as e:
            ...
            if is_live_source or url_count > 2:
                ...
                # 所有非直播源TXT（包括小说、配置文件、普通文本等）- 都使用 NOVEL_PREFIX 小说阅读器
                ...
                with open(f['path'], 'r', encoding='utf-8', errors='ignore') as ff:
                    ...
                    for line in lines:
                        ...
                        if ',#genre#' in line_lower:
                            ...
                            break
                        if 'http://' in line_lower or 'https://' in line_lower:
                            if any(ext in line_lower for ext in ['.m3u8', '.mp4', '.flv']):
                                url_count += 1
    ...
    def detailContent(self, ids):
        ...
        if id_val.startswith(self.NOVEL_PREFIX):
            ...
            return self._handle_novel_detail(file_path, id_val, encoded)
        
        if id_val.startswith(self.TEXT_PREFIX):
            encoded = id_val[len(self.TEXT_PREFIX):]
            file_path = self.b64u_decode(encoded)
            return self._handle_text_detail(file_path, id_val)
    ...
    def _handle_list_detail(self, file_path, vod_id):
        ...
        if ext == 'json':
            # 检查是否包含直播特征
            live_keywords = ['cctv', '卫视', '直播', 'live', 'm3u8', '电视台']
            content_preview = str(items)[:1000].lower()
            if any(k in content_preview for k in live_keywords):
                is_live = True
        ...
        if is_live:
            ...
            first_char = os.path.basename(file_path)[0].upper() if os.path.basename(file_path) else "J"
    ...
    def _handle_file_detail(self, file_path):
        ...
        if self.is_image_file(ext):
            ...
            is_live = False
            if preview:
                if ',#genre#' in preview or '.m3u8' in preview or 'ed2k://' in preview:
                    is_live = True
            if not is_live:
                if any(k in name.lower() for k in self.live_keywords):
                    is_live = True
    ...
    def _live_source_detail(self, source_id):
        ...
        # 构建数据 - 每个频道作为一个线路组
        from_list = []  # 线路组名称列表（频道名称）- 显示在下面
        url_list = []   # 对应的URL列表（每个频道的所有线路）- 显示在上面
        
        # 为每个频道创建一个线路组
        for channel_name, links in channels.items():
            from_list.append(channel_name)  # 线路组名称为频道名 - 显示在下面
            
            if len(links) > 1:
                # 如果这个频道有多个线路，创建多个选项
                channel_urls = []
                for i, l in enumerate(links):
                    channel_urls.append(f"线路{i+1}${l['url']}")
                url_list.append('#'.join(channel_urls))  # 所有线路用#连接 - 显示在上面
            else:
                # 如果只有一个线路，直接显示
                url_list.append(f"线路1${links[0]['url']}")
        
        vod = {
            'vod_id': self.LIVE_PREFIX + self.b64u_encode(source_id),
            'vod_name': source['name'],
            'vod_pic': icon,
            'vod_play_from': '$$$'.join(from_list),  # 所有频道名称，用$$$分隔 - 显示在下面
            'vod_play_url': '$$$'.join(url_list),     # 所有频道的线路，用$$$分隔 - 显示在上面
